...
export class LoginScreen {
    ...
    render() {
        this.container.innerHTML = `
            <div class="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950 relative overflow-hidden transition-colors duration-300">
                ...
                <!-- Login Card -->
                <div class="relative w-full max-w-md p-6 md:p-10 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border border-white/50 dark:border-gray-700/50 rounded-3xl shadow-2xl shadow-blue-900/5 fade-in mx-4">
                    ...
                    <!-- Google Login -->
                    <button id="google-login-btn" class="w-full flex items-center justify-center gap-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200 font-medium py-3 px-4 rounded-xl transition-all duration-200">
    ...
    addEventListeners() {
        ...
        const emailForm = document.getElementById('email-signin-form'); // Updated ID
        ...
        // Links
        ['link-privacy', 'link-terms', 'link-help'].forEach(id => {
            const el = document.getElementById(id);
            if (el) {
                el.addEventListener('click', (e) => {
                    e.preventDefault();
                    // Map ID to page type: link-privacy -> privacy
                    const page = id.replace('link-', '');
                    if (this.onNavigateStatic) this.onNavigateStatic(page === 'help' ? 'faq' : page);
                });
            }
        });
